 ...
 *                     \/            \/     \/    \/            \/
 * $Id: m4a.c 19146 2008-11-20 11:27:31Z zagor $
 *
...
{
    int i = read(stream->fd,buf,size);	
    if(i < 0) stream->err = LIBLOSSLESS_ERR_IO_READ;
    if(i != size) { stream->eof=1; }	
    stream->curpos += i;
...
{
  stream->curpos = lseek(stream->fd,skip,SEEK_CUR); 
  if(stream->curpos < 0) stream->err = LIBLOSSLESS_ERR_OFFSET;
...
void stream_create(stream_t *stream, msm_ctx* ctx)
{
  //  stream->ci=ci;
    stream->fd = ctx->fd;	
    stream->curpos = lseek(ctx->fd,0,SEEK_CUR);
    stream->eof=0;
    stream->err = 0;	
}
    ...
    int* current_sample)
{
    ...
    ss = lseek(stream->fd,new_pos,SEEK_SET);
